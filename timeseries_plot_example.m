...
figure
plot(featTS.timestamp(currentInds) / fps, featTS.midbody_speed(currentInds))
set(gca, 'FontSize', 18)
xlabel('Time (s)')
ylabel('Midbody Speed (\mum/s)')
...
featMean1 = grpstats(featTS.midbody_speed, featTS.timestamp, 'mean');
...
for ii = 1:numel(uniqueInds)
    ...
    featMat(ii, startInd:endInd) = featTS.midbody_speed(currentInds);
end
...
featNoZeros = double(featTS.midbody_speed);
featNoZeros(featTS.midbody_speed == 0) = eps;
...
for ii = 1:max(featTS.timestamp) + 1
    ...
    featMean4(ii) = nanmean(featTS.midbody_speed(logical(currentTimeInds)));
end
...
for ii = 1:numel(uniqueInds)
    ...
    plot(featTS.timestamp(currentInds) / fps, ...
        featTS.midbody_speed(currentInds), 'Color', [0.7 0.7 0.7])
    hold on
end
